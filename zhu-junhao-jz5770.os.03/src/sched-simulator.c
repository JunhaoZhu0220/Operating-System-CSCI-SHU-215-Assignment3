...
int main(int argc, char *argv[])
{
    ...
    else
    {
        while (fgets(line, sizeof(line), file) != NULL)
        {
            sscanf(line, "%s %u %u %u %u\n", tasks[nbOfTasks].name, &(tasks[nbOfTasks].computationTime), &(tasks[nbOfTasks].arrivalDate), &(tasks[nbOfTasks].ioDuration), &(tasks[nbOfTasks].ioInterval));
            tasks[nbOfTasks].state = UPCOMING;
            tasks[nbOfTasks].executionTime = 0;
            tasks[nbOfTasks].cyclesInIO = 0;
            nbOfTasks++;
        }
    }
    fclose(file);
