...
typedef struct task
{
    ...
    // IORR
    unsigned int ioInterval; // interval between IO operations
    unsigned int ioDuration; // duration of each IO operation
    // Used by IORR scheduler
    unsigned int cyclesInIO; // cycles spent in current IO
} task;
